...
func TestMarshalAndUnmarshal(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			g: geom.NewPolygon(geom.XY).MustSetCoords([][]geom.Coord{{{1, 2}, {3, 4}, {5, 6}, {1, 2}}, {{7, 8}, {9, 10}, {11, 12}, {7, 8}}}),
		...
		{
			g: geom.NewMultiPolygon(geom.XY).MustSetCoords([][][]geom.Coord{{{{1, 2}, {3, 4}, {5, 6}, {1, 2}}}}),
			s: "MULTIPOLYGON (((1 2, 3 4, 5 6, 1 2)))",
		},
		{
			g: geom.NewMultiPolygon(geom.XY).MustSetCoords([][][]geom.Coord{{{{1, 2}, {3, 4}, {5, 6}, {1, 2}}}, nil, {{{7, 8}, {9, 10}, {11, 12}, {7, 8}}}}),
		...
		{
			g: geom.NewGeometryCollection().MustPush(geom.NewGeometryCollection().MustSetLayout(geom.XY)).MustSetLayout(geom.XY),
